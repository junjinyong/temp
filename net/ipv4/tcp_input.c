					 ...
					 const struct tcphdr *th)
{
	...
	printk(KERN_INFO "check puzzle for %u.%u.%u.%u\n puzzle : %u, nonce : %u\n"
                , (tp->rx_opt.dns_ip       )%256
                , (tp->rx_opt.dns_ip  >>  8)%256
                , (tp->rx_opt.dns_ip  >> 16)%256
                , (tp->rx_opt.dns_ip  >> 24)%256, tp->rx_opt.puzzle, tp->rx_opt.nonce);
	return do_check_puzzle(tp->rx_opt.puzzle_type, tp->rx_opt.puzzle, tp->rx_opt.nonce, th->source);
}
